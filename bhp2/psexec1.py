...
class RemoteShell(cmd.Cmd):
    ...
    # check out impacket.smbconnection.SMBConnection.transactNamedPipe which may be viable
    def connect_listpathClient(self):
        self.listpathClient = SMBConnection('*SMBSERVER', self.server.getRemoteHost(), sess_port=self.port,
                                            preferredDialect=dialect).listPath(self.share, '/')
        user, passwd, domain, lm, nt, aesKey, TGT, TGS = self.credentials
        if self.transport.get_kerberos() is True:
            self.transferClient.kerberosLogin(user, passwd, domain, lm, nt, aesKey,
                                              kdcHost=self.transport.get_kdcHost(), TGT=TGT, TGS=TGS)
 ...
 lls {path}                 - lists contents of local directory or lists contents of local {path}
 ls {path}                  - lists contents of remote directory or lists contents of remote {path}
 lmkdir {dirname}           - makes a local directory named {dirname}
 lipconfig                  - prints output of local ipconfig (windows) or ifconfig (unix)
 lnetstat                   - prints output of local netstat 
...
""" % (self.share, self.share)
    ...
    def do_get(self, src_path):
        try:
            ...
            import ntpath
            filename = ntpath.basename(ntpath.abspath(src_path))
            fh = open(filename, 'wb')
    ...
    def do_put(self, s):
        try:
            ...
            src_file = os.path.basename(os.path.abspath(src_path))
            fh = open(os.path.abspath(src_path), 'rb')
            f = dst_path + '/' + src_file
    ...
    def do_lmkdir(self, s):
        if s == '':
            print "Need a name for directory!"
        else:
            os.mkdir(s)
        self.send_data('\r\n')

    def do_lipconfig(self, s):
        if s == '':
            if os.name == 'nt':
    ...
    def do_lnetstat(self, s):
        if s == '':
            if os.name == 'nt':
                p = subprocess.call('netstat')
                cleanp = str(p)
                print cleanp
            elif os.name == 'posix':
                p = subprocess.call('netstat')
    ...
    def do_ls(self, s):
        try:
            if s == '':
                if self.listpathClient is None:
                    listpath = self.connect_listpathClient()
                    # p = c.listPath(self.share, '')
                    cleanp = str(listpath)
                    print cleanp
            else:
                if self.transferClient is None:
                    self.connect_transferClient()
                    cmd = "powershell.exe ls %s" % s
                    p = subprocess.call(cmd)
                    cleanp = str(p)
                    print cleanp
        except Exception, e:
...
if __name__ == '__main__':
    ...
    parser.add_argument('command', nargs='*', default=' ',
                        help='command (or arguments if -c is used) to execute at the target (w/o path) - (default:cmd.exe)')
    parser.add_argument('-c', action='store', metavar="pathname",
                        help='copy the filename for later execution, arguments are passed in the command option')
