...
def main():
    ...
    parser = argparse.ArgumentParser(add_help = True, prog='python script to connect to windows via wmi', description = "Python script to connect to windows via WMI", usage='Use like so: python win32wnet.py -dev /dev/sda -img /tmp/sda1.raw')

    parser.add_argument('-h', action='store', dest='hostip', help='either hostname or ip address')
    parser.add_argument('-u', action='store', dest='usern', help='eg. administrator or pass as string to use \ in domain users')
    parser.add_argument('-p', action='store', dest='passw', help='eg. administrator or pass as string to use \ in domain users')
    ...
    parsed_host = parsed.hostip
    parsed_user = parsed.usern
    parsed_pass = parsed.passw
    # in_fd = open(str(devFile), 'r')  
    # out_fd = open(str(imgFile), 'wb')
    if parsed_host == None or parsed_user== None or parsed_pass == None:
        print options.print_usage
    else:
        # return parsed_host, parsed_user, parsed_pass
	    con = WindowsMachine(ip, username, password)
	    print con.run_remote('ipconfig', async=False, output=True)
