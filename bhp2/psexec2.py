...
""" % (self.share, self.share)
    ...
    def do_put(self, s):
        try:
            ...
            logging.info("Uploading %s to %s\%s" % (src_file, dst_path))
            self.transferClient.putFile(do_lcd(), pathname.decode(sys.stdin.encoding), fh.read)
            fh.close()
        except Exception, e:
            logging.error(str(e))
            pass
    ...
    def do_cat(self, dest_file):
        try:
            if self.transferClient is None:
                self.connect_transferClient()
                f = os.path.basename(dest_file)
                fh = open(dest_file, 'rb')    
                self.transferClient.openFile(ntpath.basename(dest_file), pathname.decode(sys.stdin.encoding), fh.read)
                fc = str(fh.read)
                print fc
                fh.close()
        except Exception, e:
    ...
    def do_edit(self, dest_file):
        try:
            if self.transferClient is None:
                self.connect_transferClient()
                f = os.path.basename(dest_file)
                fh = open(dest_file, 'wb')    
                self.transferClient.openFile(f, f.decode(sys.stdin.encoding), fh.read)
                fh.close()
        except Exception, e:
            logging.critical(str(e))
            print "Except: %s " % e

    # def write_file(self, dest_file):
    #     try:   
    #         if self.transferClient is None:
    #             self.connect_transferClient()
    #             f = os.path.basename(dest_file)
    #             fh = open(dest_file, 'wb') 
    #             self.transferClient.writeFile
    #         except Exception, e:
    #             logging.critical(str(e))
    #             print "Except: %s" % e
