...
def parse_sys_args():
    ...
    """
	rappdata = os.getenv("LOCALAPPDATA")
	mozpath = rappdata + "\Mozilla\Firefox\Profiles"
	profiles = os.walk(mozpath).next()[1]
	strprof = (str(profiles)).strip('[').strip(']').strip("'")
    profile_path = os.path.join(mozpath, strprof)
